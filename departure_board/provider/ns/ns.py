...
def departures(station_code: str) -> list:
    ...
    for departure in departures_json["payload"]["departures"]:
        if "plannedTrack" in departure:
            track = departure["plannedTrack"]
        else:
            track = ""
        ...
        trains.append(
            {
                ...
                "train_long_name": departure["name"],
                "planned_time": departure["plannedDateTime"][11:16],
                "actual_time": departure["actualDateTime"][11:16],
                "track": track,
                "train_number": departure["product"]["number"],
                "train_category": departure["product"]["longCategoryName"],
                "train_operator": departure["product"]["operatorName"],
            }
        )
