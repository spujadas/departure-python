...
def timetable_from_departure(departure):
    timetable = []

    # schedule if disruption
    has_disruption = False
    for link in departure["display_informations"]["links"]:
...
def next_trains(stop_area_id: str, timetable_for_all_trains: bool = False):
    ...
    for i, departure in enumerate(departures["departures"]):
        ...
        base_departure_date_time = departure["stop_date_time"][
        ...
        if i == 0 or timetable_for_all_trains:
            train_data["timetable"] = timetable_from_departure(departure)
