...
import departure_board.board.view_model as view_model
import departure_board.board.contents as contents
import departure_board.helpers as helpers
import departure_board.board.departure_board_pb2 as departure_board_pb2
import departure_board.board.departure_board_pb2_grpc as \
    departure_board_pb2_grpc  # for type hinting
import departure_board.board.protobuf as board_protobuf
...
class ViewModelRatp_192_32_3_Rows_To_ProtocolBuffers(ViewModelRatp_192_32):
    def __init__(self, board_manager_stub: departure_board_pb2_grpc.BoardManagerStub):
        super().__init__()
        self.board_manager_stub = board_manager_stub

        self.next_mission_tracker = [
            departure_board_pb2.Movement(
                static_content=departure_board_pb2.StaticContent(total_duration=2000)
            ),
            departure_board_pb2.Movement(
                scrolling_content=departure_board_pb2.ScrollingContent(
                    step_duration=100,
                    total_steps=11,
                    delta_x_per_step=0,
                    delta_y_per_step=-1,
        ...
        self.no_movement = [
            departure_board_pb2.Movement(no_movement=departure_board_pb2.NoMovement())
        ]
    ...
    def update(self, departures):  # pylint: disable=arguments-differ
        ...
        requests_data.append(
            departure_board_pb2.BoardSectionUpdateRequest(
                section_index=0,
                content=departure_board_pb2.BoardSectionContent(
        ...
        try:
            self.board_manager_stub.BoardSectionsUpdate(
                departure_board_pb2.BoardSectionsUpdateRequest(requests=requests_data)
            )
