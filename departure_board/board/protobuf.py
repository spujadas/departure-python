...
def deserialise_Pixel(pixel: departure_board_pb2.Pixel):
    return pixel.x, pixel.y, (pixel.r, pixel.g, pixel.b)


def deserialise_BoardSectionContent(
    board_section_content: departure_board_pb2.BoardSectionContent,
...
def deserialise_NoMovement(
    no_movement: departure_board_pb2.NoMovement,
) -> movement.NoMovement:
...
def deserialise_StaticContent(
    static_content: departure_board_pb2.StaticContent,
...
def deserialise_Movement(
    single_movement: departure_board_pb2.Movement,
...
def deserialise_RepeatedMovement(
    movement_list: List[departure_board_pb2.Movement],
    ...
    for m in movement_list:
        movement_segment = deserialise_Movement(m)
...
def serialise_pixels(pixels) -> List[departure_board_pb2.Pixel]:
    serialised_pixels = []
    for x, y, c in pixels:
        serialised_pixels.append(
            departure_board_pb2.Pixel(x=x, y=y, r=c[0], g=c[1], b=c[2])
        )
