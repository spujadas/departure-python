...
class BoardUpdater_192_32_3_Rows_From_ProtocolBuffers(BoardUpdater_192_32_3_Rows):
    def update(
        self,
        board_sections_update_request: departure_board_pb2.BoardSectionsUpdateRequest,
    ) -> departure_board_pb2.BoardSectionsUpdateResponse:
        ...
        for req in board_sections_update_request.requests:
            if req.section_index == 0:
                ...
                sections_update_response.append(
                    ...
                    departure_board_pb2.BoardSectionOperationStatus(
                        section_index=req.section_index,
                        status=departure_board_pb2.BoardSectionOperationStatus.SECTION_NOEXISTS,
                    )
                )
                continue
            ...
            sections_update_response.append(
                departure_board_pb2.BoardSectionOperationStatus(
                    section_index=req.section_index,
                    status=departure_board_pb2.BoardSectionOperationStatus.OK,
                )
            )

        self.update_sections(**sections_update_data)

        return departure_board_pb2.BoardSectionsUpdateResponse(
